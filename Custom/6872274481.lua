...
runcode(function()
	local bestdisablerreal = {["Enabled"] = false}
	bestdisablerreal = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		["Name"] = "HangGliderExploit",
		["HoverText"] = "useless exploit lmao (Hang Glider needed)",
		["Function"] = function(callback)
			if callback then
				local args = {
					[1] = {}
				...
				game:GetService("ReplicatedStorage").rbxts_include.node_modules.net.out._NetManaged.HangGliderUse:FireServer(unpack(args))
				bestdisablerreal["ToggleButton"](false)
			end
		end 
	})
end)
...
runcode(function()
	local HighJumpMode = {["Value"] = "Normal"}
	local HighJumpBoost = {["Value"] = 10}
	local HighJumpDelay = {["Value"] = 5}
	local HighJumpTick = tick()
	local highjumpbound = true
	local HighJump = {["Enabled"] = false}
	HighJump = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
